...
class GridScene(QGraphicsScene):
    ...
    def set_background_from_array(self, arr: np.ndarray) -> None:
        ...
        # Transpose to fix diagonal mirroring: swap rows and columns
...
class MainWindow(QMainWindow):
    def __init__(self) -> None:
        ...
        self.btn_dummy_pcl = QPushButton("Dummy PCL")
        ...
        self.btn_load.clicked.connect(self.on_load_map)
        ...
        self.btn_dummy_pcl.clicked.connect(self.on_dummy_pcl)
        ...
        side_layout.addWidget(self.lbl_min)
        side_layout.addWidget(self.lbl_max)
    ...
    def on_toggle_desired_elevation(self, state: int) -> None:
        """Toggle between showing elevation and desired_elevation."""
        if state == Qt.Checked and self.desired_elevation_canvas is None:
            QMessageBox.information(
                self,
                "No Desired Elevation",
                "No desired_elevation layer found in the loaded bag file.\n\n"
    ...
    def on_load_map(self) -> None:
        ...
        try:
            ...
            # Store as canvas and use placement system to display on grid
            self.last_pcl_canvas = resized
            self.offset_x = 0
            self.offset_y = 0
    ...
    def on_load_foundation(self) -> None:
        ...
        try:
            ...
            if faces is not None and len(faces) > 0:
                ...
                # For imported foundations, convert absolute Z to relative depth (below surface)
                if self.last_placed_elevation is not None:
                    for (cx, cy), z_values in cells_to_fill.items():
                        if 0 <= cy < self.grid_size and 0 <= cx < self.grid_size:
                            ...
                            # Convert to relative depth: surface elevation - bottom Z
                            surface_z = self.last_placed_elevation[cy, cx]
                            relative_depth = surface_z - avg_bottom_z
                            # Store as relative depth (negative means below surface)
                            if self.scene.foundation_depth_map is None:
                                self.scene.foundation_depth_map = np.zeros((self.grid_size, self.grid_size), dtype=np.float32)
                            self.scene.foundation_depth_map[cy, cx] = max(0.0, relative_depth)
                else:
                    # No elevation data, store absolute Z (will be converted later)
                    for (cx, cy), z_values in cells_to_fill.items():
                        if 0 <= cy < self.grid_size and 0 <= cx < self.grid_size:
                            self.scene.foundation_mask[cy, cx] = 1
                            ...
                            if self.scene.foundation_depth_map is None:
                                self.scene.foundation_depth_map = np.zeros((self.grid_size, self.grid_size), dtype=np.float32)
                            self.scene.foundation_depth_map[cy, cx] = avg_z
            else:
                ...
                # Store average Z per cell, convert to relative depth if elevation data available
                if self.last_placed_elevation is not None:
                    ...
                    # No elevation data, store absolute Z
                    for (cx, cy), z_values in cell_z_map.items():
                        if 0 <= cy < self.grid_size and 0 <= cx < self.grid_size:
                            if self.scene.foundation_depth_map is None:
                                self.scene.foundation_depth_map = np.zeros((self.grid_size, self.grid_size), dtype=np.float32)
                            self.scene.foundation_depth_map[cy, cx] = np.mean(z_values)
    ...
    def on_load_geo_map(self) -> None:
        ...
        file_path, _ = QFileDialog.getOpenFileName(
            ...
            os.getcwd(),
            "ROS Bag Files (*.bag);;NumPy Files (*.npy);;All Files (*.*)"
        )
        if not file_path:
            return
        ...
        try:
            ...
            if not config.get('gnss', {}).get('useGnssReference', False):
                QMessageBox.warning(
                    self,
                    "Config Warning",
            ...
            if file_ext == '.npy':
                ...
                self.desired_elevation_canvas = None
                self.original_desired_elevation_array = None
            ...
            # Handle .bag files (original code)
            if not HAS_ROSBAGS:
                QMessageBox.critical(
                    self,
                    "Missing Dependency",
                    "rosbags library is required to load .bag files.\n\n"
            ...
            with AnyReader([bag_path_obj]) as reader:
                conns = [c for c in reader.connections if c.topic == "grid_map"]
                if not conns:
                    raise ValueError("No 'grid_map' topic found in bag file")
                ...
                # Extract desired_elevation layer if it exists and compare with elevation
                desired_elev_array = None
                if "desired_elevation" in layers:
                    ...
                    if desired_elev_idx < len(data):
                        ...
                        if isinstance(desired_elev_matrix, np.ndarray):
                            desired_elev_array = desired_elev_matrix.astype(np.float32)
                            ...
                            # Get shape from layout if available (same as elevation)
                            desired_shape = None
                            ...
                            if isinstance(raw_data, np.ndarray):
                                desired_elev_array = raw_data.astype(np.float32)
                            elif isinstance(raw_data, (list, tuple)):
                                desired_elev_array = np.array(raw_data, dtype=np.float32)
                            elif hasattr(raw_data, '__iter__'):
                                # Try to convert iterable to list then array
                                desired_elev_array = np.array([float(x) for x in raw_data], dtype=np.float32)
                            
                            # Reshape using layout information if available
                            if desired_elev_array is not None and desired_shape is not None:
                                if desired_elev_array.size == desired_shape[0] * desired_shape[1]:
                                    desired_elev_array = desired_elev_array.reshape(desired_shape)
                        
                        # Reshape to match elevation shape (must be done after extraction)
                        if desired_elev_array is not None:
                            if desired_elev_array.ndim == 1:
                                # 1D array - reshape to match elevation shape
                                if desired_elev_array.size == H * W:
                                    desired_elev_array = desired_elev_array.reshape((H, W))
                                else:
                                    print(f"Warning: desired_elevation size ({desired_elev_array.size}) doesn't match elevation size ({H * W})")
                                    desired_elev_array = None
                            elif desired_elev_array.ndim == 2:
                                # Already 2D - check if shape matches
                                if desired_elev_array.shape != (H, W):
                                    print(f"Warning: desired_elevation shape {desired_elev_array.shape} doesn't match elevation shape ({H}, {W})")
                        ...
                        # Handle NaN/infinite values
                        if desired_elev_array is not None:
                            desired_elev_array = np.where(np.isfinite(desired_elev_array), desired_elev_array, 0.0)
                            
                            # Ensure shapes match before computing difference
                            if desired_elev_array.shape == elev_array.shape:
                                # Compute and print difference statistics
                                diff = desired_elev_array - elev_array
                                finite_diff = diff[np.isfinite(diff)]
                                
                                if finite_diff.size > 0:
                                    # Count significant differences (above floating point precision threshold)
                                    significant_diff = np.abs(finite_diff) > 1e-6
                                    num_significant = np.count_nonzero(significant_diff)
                                    
                                    print(f"\n=== Elevation vs Desired Elevation Comparison ===")
                                    print(f"Elevation range: {elev_array.min():.3f}m to {elev_array.max():.3f}m")
                                    print(f"Desired elevation range: {desired_elev_array.min():.3f}m to {desired_elev_array.max():.3f}m")
                                    print(f"Difference statistics:")
                                    print(f"  Min difference: {finite_diff.min():.3f}m")
                                    print(f"  Max difference: {finite_diff.max():.3f}m")
                                    print(f"  Mean difference: {finite_diff.mean():.3f}m")
                                    print(f"  Std deviation: {finite_diff.std():.3f}m")
                                    print(f"  Non-zero differences (>1e-6m): {num_significant} / {finite_diff.size} ({100.0 * num_significant / finite_diff.size:.1f}%)")
                                    print(f"  Cells with difference > 0.01m: {np.count_nonzero(np.abs(finite_diff) > 0.01)} ({100.0 * np.count_nonzero(np.abs(finite_diff) > 0.01) / finite_diff.size:.1f}%)")
                                    print(f"  Cells with difference > 0.1m: {np.count_nonzero(np.abs(finite_diff) > 0.1)} ({100.0 * np.count_nonzero(np.abs(finite_diff) > 0.1) / finite_diff.size:.1f}%)")
                                    print(f"==================================================\n")
                                else:
                                    print("Warning: Could not compute difference statistics (no finite values)")
                            else:
                                print(f"Warning: desired_elevation shape {desired_elev_array.shape} doesn't match elevation shape {elev_array.shape}, skipping comparison")
                        else:
                            print("Warning: Could not extract desired_elevation data")
                else:
                    print("Info: No 'desired_elevation' layer found in bag file")
                ...
                self.original_elevation_array = elev_array.copy()
                
                # Resize and store desired_elevation if it exists
                if desired_elev_array is not None and desired_elev_array.shape == elev_array.shape:
                    ...
                    self.original_desired_elevation_array = desired_elev_array.copy()
                    ...
                    self.desired_elevation_canvas = desired_resized
                else:
                    # No desired_elevation or shape mismatch - clear it
                    self.desired_elevation_canvas = None
                    self.original_desired_elevation_array = None
                
                # Update checkbox state based on availability
                if hasattr(self, 'chk_show_desired_elevation'):
                    self.chk_show_desired_elevation.setEnabled(self.desired_elevation_canvas is not None)
                    if self.desired_elevation_canvas is None:
                        self.chk_show_desired_elevation.setChecked(False)
    ...
    def on_load_pcl(self) -> None:
        ...
        try:
            ...
            self.last_pcl_canvas = canvas
            self._update_offset_ranges()
            self._apply_current_placement()
        except Exception as exc:
            QMessageBox.critical(self, "PCL Load Error", f"Failed to load point cloud:\n{exc}")

    def on_dummy_pcl(self) -> None:
        tiles = 100
        step = self.meters_per_tile
        xs = np.arange(0, tiles) * step
        ys = np.arange(0, tiles) * step
        X, Y = np.meshgrid(xs, ys)
        slope = 0.002 * (X - X.mean()) + 0.0015 * (Y - Y.mean())
        terrain = (
            0.8 * np.sin(X * 0.15) * np.cos(Y * 0.11) +
            0.4 * np.sin(X * 0.05 + 1.3) * np.sin(Y * 0.07 + 0.7) +
            0.2 * np.cos(X * 0.21 - 0.4) * np.cos(Y * 0.17 + 0.2)
        )
        cx, cy = xs.mean(), ys.mean()
        R = np.sqrt((X - cx) ** 2 + (Y - cy) ** 2)
        hill = 1.2 * np.exp(-(R / (tiles * step * 0.45)) ** 2)
        Z = 0.8 * terrain + hill + slope
        zmin, zmax = float(Z.min()), float(Z.max())
        if zmax - zmin > 1e-6:
            Z = 2.0 * (Z - zmin) / (zmax - zmin)
    ...
    def _apply_current_placement(self) -> None:
        # Use desired_elevation canvas if toggle is on and available
        canvas_to_use = None
        if (hasattr(self, 'chk_show_desired_elevation') and 
            self.chk_show_desired_elevation.isChecked() and 
            self.desired_elevation_canvas is not None):
        ...
        print(f"_apply_current_placement: mode_text='{self.placement_combo.currentText()}' -> mode='{mode}', offx={offx}, offy={offy}")
    ...
    def update_foundation_profile(self) -> None:
        ...
        # Bottom is flat at (max elevation within mask) - depth
        max_in_mask = float(sub_elev[sub_mask].max()) if sub_mask.any() else float(sub_elev.max())
        bottom_height = max_in_mask - depth
        dig = np.maximum(0.0, sub_elev - bottom_height)
        dig = np.where(sub_mask, dig, 0.0)
        self.lbl_min.setText(f"Min dig: {dig[sub_mask].min() if sub_mask.any() else 0.0:.2f} m")
        self.lbl_max.setText(f"Max dig: {dig[sub_mask].max() if sub_mask.any() else 0.0:.2f} m")
        ...
        def draw_profile(scene: QGraphicsScene, profile_arr: np.ndarray, title_text: str) -> None:
            ...
            t_depth = scene.addSimpleText(f"Depth: {depth:.2f} m")
    ...
    def _export_to_isaac(self, out_dir: str) -> None:
        ...
        if hasattr(self, 'original_elevation_array') and self.original_elevation_array is not None:
            elev = self.original_elevation_array.copy()
            ...
            if hasattr(self, 'placement_params') and self.placement_params is not None:
                ...
                if stored_canvas_w != resized_W or stored_canvas_h != resized_H:
                    ...
                    # Use stored canvas dimensions for bounds checking if they differ
                    bounds_w = stored_canvas_w
                    bounds_h = stored_canvas_h
                else:
                    bounds_w = resized_W
                    bounds_h = resized_H
                ...
                if mode == "topleft":
                    ...
                    # If consistently 3 tiles off, adjust start coordinates
                    # This accounts for any systematic offset in the coordinate mapping
                    if start_x == test_start_x and start_y == test_start_y:
                        print(f"  NOTE: Stored values match apply_placement calculation.")
                        # Apply +3 offset to correct for consistent 3-tile offset (too far right/up means we need to add)
                        start_x = start_x - 2
                        start_y = start_y + 2
                        print(f"  Adjusted start coordinates by +3: new start=({start_x},{start_y})")
            else:
            ...
            def map_mask_to_original(mask_placed, label):
                ...
                for cy in range(placed_H):
                    for cx in range(placed_W):
                        ...
                        if resized_cy < 0 or resized_cy >= bounds_h or resized_cx < 0 or resized_cx >= bounds_w:
                            continue
                        ox_start = int(round((resized_cy * mpt) / orig_resolution))
                        ox_end = int(round(((resized_cy + 1) * mpt) / orig_resolution))
                        oy_start = int(round((resized_cx * mpt) / orig_resolution))
                        oy_end = int(round(((resized_cx + 1) * mpt) / orig_resolution))
                        if ox_end <= ox_start:
                            ox_end = ox_start + 1
                        if oy_end <= oy_start:
                            oy_end = oy_start + 1
            ...
            foundation_mask, _ = map_mask_to_original(foundation_mask_placed, "foundation")
            ...
            if obstacle_mask_placed is not None:
                ...
                obstacle_mask_original, obstacle_original_tiles = map_mask_to_original(obstacle_mask_bool, "obstacle/occupancy")
            ...
            if depth_map_placed is not None:
                ...
                # Iterate through placed grid and map each non-zero cell to original grid
                for cy in range(placed_H):
                    for cx in range(placed_W):
                        if foundation_mask_placed[cy, cx] and depth_map_placed[cy, cx] != 0:  # Only map where foundation exists and has depth
                            # Map from placed grid to resized canvas coordinates (same as foundation mask)
                            resized_cy = start_y + cy
                            resized_cx = start_x + cx
                            
                            # Skip if outside resized canvas bounds (use stored canvas dimensions if available)
                            if resized_cy < 0 or resized_cy >= bounds_h or resized_cx < 0 or resized_cx >= bounds_w:
                                continue
                            
                            # Map from resized canvas to original using physical coordinates with coordinate swap
                            # Use the same mpt and orig_resolution from above
                            ox_start = int(round((resized_cy * mpt) / orig_resolution))  # resized_cy (row) -> ox (column)
                            ox_end = int(round(((resized_cy + 1) * mpt) / orig_resolution))  # End is exclusive
                            oy_start = int(round((resized_cx * mpt) / orig_resolution))  # resized_cx (column) -> oy (row)
                            oy_end = int(round(((resized_cx + 1) * mpt) / orig_resolution))  # End is exclusive
                            # Ensure at least one cell is covered
                            if ox_end <= ox_start:
                                ox_end = ox_start + 1
                            if oy_end <= oy_start:
                                oy_end = oy_start + 1
                            ...
                            ox_start = max(0, min(ox_start, orig_W - 1))
                            ox_end = max(0, min(ox_end, orig_W))
                            # Set all cells in this block to the same depth value
                            depth_map[oy_start:oy_end, ox_start:ox_end] = depth_map_placed[cy, cx]
        else:
        ...
        if self.original_bag_path and self.original_gridmap_msg and HAS_ROSBAGS:
            # Only create bag file if original was a bag file
            ...
            if original_ext == '.bag':
                try:
                    ...
                    # Try to import writers - prefer rosbag1 (single file) over rosbag2 (directory)
                    Writer = None
                    use_anywriter = False
                    use_rosbag1 = False
                    
                    # First try rosbag1.Writer (creates single .bag file)
                    try:
                    ...
                    # Create output bag file name
                    original_name = os.path.splitext(os.path.basename(self.original_bag_path))[0]
                    bag_path = os.path.join(out_dir, f"{original_name}_modified.bag")
                    ...
                    # Convert elevation to numpy array (keep as array, don't convert to list)
                    elev_array = elev.astype(np.float32)
                    elev_flat = elev_array.ravel()
                    ...
                    layer_targets = [{
                        ...
                        'flat': elev_flat,
                        'shape': elev_array.shape
                    }]
                    if occupancy_array is not None:
                        if "occupancy" not in layers:
                            layers.append("occupancy")
                        occupancy_idx = layers.index("occupancy")
                        occupancy_flat = occupancy_array.astype(np.float32).ravel()
                    ...
                    def build_modified_message():
                        ...
                        if hasattr(msg_copy, 'data') and layer_targets:
                            ...
                            while len(data_list) <= max_target_idx:
                                new_layer = None
                                if elev_layer_data is not None and hasattr(elev_layer_data, '__class__'):
                                    new_layer = elev_layer_data.__class__()
                                    if hasattr(new_layer, 'data'):
                                        new_layer.data = []
                                    if hasattr(new_layer, 'layout') and hasattr(elev_layer_data, 'layout'):
                                        new_layer.layout = copy_module.deepcopy(elev_layer_data.layout)
                                elif len(data_list) > 0:
                                    first_layer = data_list[0]
                                    if hasattr(first_layer, '__class__'):
                                        new_layer = first_layer.__class__()
                                        if hasattr(new_layer, 'data'):
                                            new_layer.data = []
                                        if hasattr(new_layer, 'layout') and hasattr(first_layer, 'layout'):
                                            new_layer.layout = copy_module.deepcopy(first_layer.layout)
                    ...
                    if use_rosbag1:
                        ...
                        writer = Writer(Path(bag_path), version=9)  # version 9 is rosbag2 format
                        writer.open()
                        try:
                            # Add connection for grid_map topic
                            conn_kwargs = {
    ...
    def _build_bottom_bar(self) -> QWidget:
        ...
        if not hasattr(self, 'btn_load_geo'):
            self.btn_load_geo = QPushButton("Load Geo Map")
            self.btn_load_geo.clicked.connect(self.on_load_geo_map)
        if not hasattr(self, 'btn_dummy_pcl'):
            self.btn_dummy_pcl = QPushButton("Dummy PCL")
            self.btn_dummy_pcl.clicked.connect(self.on_dummy_pcl)
        ...
        if not hasattr(self, 'placement_combo'):
            ...
            self.placement_combo.setCurrentText("Top-Left")
            self.placement_combo.currentTextChanged.connect(self.on_placement_changed)
        # Add export target selector
        if not hasattr(self, 'export_target_combo'):
            self.export_target_combo = QComboBox()
            self.export_target_combo.addItems(["Terra", "Isaac Sim"])
        ...
        bottom_layout.addWidget(self.btn_dummy_pcl)
    ...
    def on_group_depth_changed(self, value: float) -> None:
        """Update depth for selected foundation group - scale proportionally."""
        ...
        if max_current_depth > 1e-6:
            scale_factor = new_max_depth / max_current_depth
        else:
            # No existing depth, use uniform depth
            scale_factor = 1.0
        
        # Update all depths proportionally
        # Update both scene depth map and group depth map
        for x, y in cells:
            if 0 <= x < self.scene.grid_size and 0 <= y < self.scene.grid_size:
                old_depth = current_depths.get((x, y), 0.0)
                new_depth = old_depth * scale_factor
        ...
        # Update 3D view to reflect new depth (clears old walls/bottoms and rebuilds)
        self.update_3d_view()
        self.update_foundation_profile()
